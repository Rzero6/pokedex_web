  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <div ref={carouselRef} className="overflow-hidden h-full w-full">
        <div
          ref={ref}
          className={cn("flex h-full w-full", orientation === "horizontal" ? "-ml-4" : "-mt-4 flex-col", className)}
          {...props}
        />
      </div>
    );
...
const CarouselItem = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, onClick, ...props }, ref) => {
    ...
    const handleItemClick = (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (clickX < width / 2) scrollPrev();
      else scrollNext();

      // If the dev passed a custom click handler, still call it:
      ...
      {Array.from({ length: slideCount }).map((_, index) => (
        <button
          ...
          className={cn(
            ...
            index === selectedIndex ? "opacity-100 bg-white" : "opacity-40 bg-white/70"
          )}
